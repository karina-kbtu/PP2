        ...
        try:
            with psycopg2.connect(**config) as conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT user_id,name,phone_number FROM phonebook1 ORDER BY user_id")
                    rows = cur.fetchall()
...
def delete_info(user_id):
    """ Удаление контакта """
    rows_deleted = 0
    sql = 'DELETE FROM phonebook1 WHERE user_id = %s'
    config = load_config()
...
def insert_contact(name, phone_number):
    ...
    sql = """INSERT INTO phonebook1("name", "phone_number")
